...
public class FrmOrder_collp extends JDialog {
    public FrmOrder_collp(String title)
    {
    ...
    public static CategoryDataset createDataset() //创建柱状图数据集
    {
        ...
        String sql = "SELECT name,sum(number) FROM mydb.productionorder,production where productionorder.productionid=production.id and Date>? group by name";
        try {
    ...
    {
        JFreeChart chart= ChartFactory.createBarChart("hi", "产品分布",
                "产品出售数量", dataset, PlotOrientation.VERTICAL, false, false, false); //创建一个JFreeChart
        chart.setTitle(new TextTitle("近一月产品出售统计",new Font("宋体",Font.BOLD+Font.ITALIC,20)));//可以重新设置标题，替换“hi”标题
